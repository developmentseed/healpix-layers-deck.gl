...
console.log(`  f_row=${Math.floor(f / 4)} (0=north cap, 1=equator, 2=south cap)`);
...
for (let ci = 0; ci < 4; ci++) {
  ...
  const branch = Math.abs(u) > PI_4 ? 'POLAR' : Math.abs(u) >= PI_2 ? 'POLE' : 'EQUATORIAL';
  console.log(`  ci=${ci}  cx=${cx} cy=${cy}  i_ring=${i_ring} k_ring=${k_ring}`);
  console.log(`         t=${t_unwrapped.toFixed(6)} u=${u.toFixed(6)}  branch=${branch}`);
}
